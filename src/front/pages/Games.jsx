...
export const Games = () => {
      ...
      const enrichedGames = await Promise.all(
        games.map(async (game) => {
          try {
            ...
            console.error(
              `Failed to fetch description for ${game.name}`,
              error
            );
  ...
  return (
    ...
    >
      <h1>A selection of Modern Game</h1>

      <div className="row">
        {gamesWithDescriptions.map((rawgGame) => (
          <div key={rawgGame.id} className="col-6 col-md-4 col-lg-3 mb-4">
          <Link to={`/game/${rawgGame.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
            <RawgGameCard
